...
DenseLayer::DenseLayer(int batch_size, int input_size, int output_size) : _h_batch_size(batch_size),
...
const float* DenseLayer::Forward(const float* d_input, std::unique_ptr<Activation> activation) {
    // Clear the gradients
...
const float* DenseLayer::Backward(const float* d_dZ, const float* d_activation_prev_layer) {
...
void DenseLayer::Update(float learning_rate, cudaStream_t stream) {
    const int threadsPerBlock = 256;
...
std::vector<float> DenseLayer::GetWeightsCPU() const {
...
std::vector<float> DenseLayer::GetBiasCPU() const {
...
std::vector<float> DenseLayer::GetOutputCPU() const {
...
std::vector<float> DenseLayer::GetBiasGradientsCPU() const {
