...
void FMNISTDataSet::ReadFMNISTData(const std::filesystem::path& image_file_path, const std::filesystem::path& label_file_path) {
    // Open files
    ...
    if (image_file.is_open() && label_file.is_open()) {
        //Magic number use to classify the file type (2051 is image file and 2049 is label file). First 4 bytes of files
...
uint32_t FMNISTDataSet::flipEndianness(const uint32_t u) {
